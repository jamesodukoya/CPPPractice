...
int main() {
    ...
    std::replace_if(my_vector.begin(), my_vector.end(), [](const auto val) { return val % 2 == 0; }, -1);
    print_vector(my_vector);

    std::sort(my_vector.begin(), my_vector.end(), [](const auto i, const auto j) { return i < j; });
    print_vector(my_vector);
