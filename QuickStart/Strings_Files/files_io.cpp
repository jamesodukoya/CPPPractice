#include <fstream> // For file stream (ifstream, ofstream)
#include <iostream>
#include <string>
...
int write_line_to_file(std::string_view filepath, const std::string &line) {
    auto file = std::ofstream{}; // Create an output filestream object for writing to the file

    // Open file for output/writing purpose, discards the file if it exists
    file.open(filepath.data(), std::ios::out | std::ios::app); // ::app flag means append
    if (file.fail())
        return 1;
    ...
    // file.close(); // Not needed because modern C++ does RAII
